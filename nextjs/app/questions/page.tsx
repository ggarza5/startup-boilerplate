...
const IndexPage: React.FC = () => {
  ...
  const [isFetchingSection, setisFetchingSection] = useState(false); // Track section creation state
  const router = useRouter(); // Initialize useRouter

  useEffect(() => {
    // Fetch sections from API or database
    const fetchSections = async () => {
      const response = await fetch('/api/sections'); // Fetch sections from the new GET handler
  ...
  const handleSelectSection = async (sectionName: string, sectionId: string) => { // Change parameter type to string
    setLoading(true); // Set loading to true
    setisFetchingSection(true); // Set creating section state
    try {  
      const response = await fetch(`/api/sections/${sectionName}`); // Fetch specific section details using name
      const data = await response.json();  
      setCurrentQuestionIndex(0); // Reset question index
      setCurrentSection(data); // Set the current section
      setStartTimer(true); // Start the timer
    } catch (error) {
      console.error('Error fetching section:', error);
    } finally {
      setLoading(false); // Set loading to false
      setisFetchingSection(false); // Reset creating section state
    }
  };
  ...
  const handleNextQuestion = () => {
    if (currentSection && currentQuestionIndex < currentSection.questions.length - 1) {
      ...
      resetSelectedAnswer(); // Reset selected answer
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="flex grow">
        <Sidebar sections={sections} onSelectSection={handleSelectSection} /> {/* Add Sidebar */} 
        <div className="flex grow flex-col p-4"> {/* Add margin to avoid overlap */}
          {loading || isFetchingSection ? (
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div className="flex justify-between">
                  ...
                  {currentSection.questions && currentQuestionIndex < currentSection.questions.length - 1 ? (
                    ...
                    <button
                      onClick={() => handleSubmitAnswers({})} // Pass the answers object
