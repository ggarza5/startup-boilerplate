...
export class SeoBotClient {
  ...
  private SEOBotBaseURL = "https://seobot-blogs.s3.eu-north-1.amazonaws.com";

  private async fetchBase() {
    const base = await this.fetchBaseCache.get(async () => {
      const response = await fetch(`${this.SEOBotBaseURL}/${this.key}/system/base.json`, { cache: "no-store" });
  ...
  private async fetchPost(id: string) {
    const post = await this.fetchPostCache.get(async () => {
      const postData = await fetch(`${this.SEOBotBaseURL}/${this.key}/blog/${id}.json`, { cache: "no-store" });
  ...
  async getCategoryPosts(slug: string, page: number, limit: number = 10) {
    try {
      const base = await this.fetchBase();
      const categoryIds = base.filter((item: any) => item?.category?.slug == slug);
      const start = page * limit;
      const end = start + limit;
      const articles = await Promise.all(
